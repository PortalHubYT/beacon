...
class Guesses(Portal):
    ...
    async def loop(self):
        if self.chat_queue:
            ...
            #those commands in command block make chicken move and die 
            #/kill @e[type=minecraft:text_display, distance=0..2]
            # /kill @e[type=minecraft:chicken, distance=0..2]
            # execute as @e[type=chicken] at @s run tp ~0.1 ~ ~
