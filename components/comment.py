...
class Component(ApplicationSession):
    async def onJoin(self, details):
        ...
        def on_message(profile):
            ...
            print(
                f"-> normal: {len(normal_queue)}, follower: {len(follower_queue)}, donator: {len(priority_queue)} forced: {len(bypass_queue)} total: {len(normal_queue) + len(follower_queue) + len(priority_queue) + len(bypass_queue)}"
            )
